...
public class Player : MonoBehaviour
{
    ...
    MinimaxTree<Configuration> BuildTree(Configuration boardConfiguration)
    {
        ...
        MinimaxTree<Configuration> tree = new MinimaxTree<Configuration>(boardConfiguration);
        nodeList.Clear();
        ...
        while (nodeList.Count > 0)
        {
            ...
            foreach (Configuration child in children)
            {
                ...
                int CurrentTreeDeapth = 1;
                MinimaxTreeNode<Configuration> childNode = new MinimaxTreeNode<Configuration>(child, currentNode);
            
                MinimaxTreeNode<Configuration> DepthNode = childNode;

                while (DepthNode.Parent != currentNode)
                {
                    DepthNode = childNode.Parent;
                    CurrentTreeDeapth++;
                }

                if (CurrentTreeDeapth <= searchDepth) 
                {
    ...
    void HandleThinkingTimerFinished()
    {
        ...
        IList<MinimaxTreeNode<Configuration>> children = tree.Root.Children;
    ...
    List<Configuration> GetNextConfigurations(Configuration currentConfiguration)
    {
    ...
    void AssignHeuristicMinimaxScore(MinimaxTreeNode<Configuration> node, bool maximizing)
    {
        ...
        {
            // I really , Really didn't understand this concept or how it ould work please redirect me to a resourse
